...
{
    [m_HUD setHidden: NO];
    [m_HUD.label setText: @"Load device dtat ..."];
    NSError *error;
...
{
    [m_HUD setHidden: NO];
    m_HUD.label.text = @"load detail data ...";
    NSError *error;
    ...
    [[session dataTaskWithRequest: request_user_info
                completionHandler:^(NSData *data, NSURLResponse *response, NSError *error)
      {
          ...
          //if (DEBUG) debug(@"device detail info = %@", resellerInfo);
...
- (void)editSearchResellerInfo
{
    ...
    if (self.editViewResellerName.text.length > 0)
    {
        NSError *error;
        NSString *payloadFormat = [NSString stringWithFormat: @"{"
                                   "\"reseller_search\": \"%@\""
...
//                               "\"licenses\": [{"
//                               "\"key\": \"%@\""
//                               "}]}", detailDeviceId, licenseKey];
//    NSData *payloadJson = [payloadFormat dataUsingEncoding: NSUTF8StringEncoding];
//    NSDictionary *payload = [NSJSONSerialization JSONObjectWithData: payloadJson options: kNilOptions error: &error];
//    debug(@"sfafasf = %@", payload);
//    NSString *token = [Jwt encodeWithPayload: payload andKey: [public get_secret_access_key] andError:&error];
//    if(token == nil)
//    {
//        if (DEBUG) debug(@"Code: %li", (long)[error code]);
...
//    }
//    
//    NSString *update_device_info_url = [NSString stringWithFormat: @"%@/api/v2/my/devices/licenses/activate?token=%@&access_key_id=%@", DATA_URL, token, [public get_access_key_id]];
//    if (DEBUG) debug(@"update_device_info_url = %@", update_device_info_url);
//    NSURL *url = [NSURL URLWithString: update_device_info_url];
//    NSMutableURLRequest *request_user_info = [NSMutableURLRequest requestWithURL: url cachePolicy: NSURLRequestUseProtocolCachePolicy timeoutInterval: 10];
//    [request_user_info setHTTPMethod: @"POST"];
...
//              NSMutableDictionary *reseller_info_json = [NSJSONSerialization JSONObjectWithData: aes_decode_data options:kNilOptions error: nil];
//              
//              NSArray *resellerListArr = [reseller_info_json objectForKey: @"licenses"];
//              for (NSDictionary *license in resellerListArr)
//              {
//                  NSData *return_status = [NSJSONSerialization dataWithJSONObject: [license objectForKey: @"return_status"] options: NSJSONWritingPrettyPrinted error: nil];
//                  NSMutableDictionary *status = [NSJSONSerialization JSONObjectWithData: return_status options:kNilOptions error: nil];
//                  
//                  NSString *return_code = [NSString stringWithFormat: @"%@", [status objectForKey: @"code"]];
//                  NSString *return_message = [NSString stringWithFormat: @"%@", [status objectForKey: @"message"]];
//                  if ([return_code isEqualToString: @"0"])
//                  {
//                      NSString *licenseKey = [NSString stringWithFormat: @"%@", [license objectForKey: @"key"]];
//                      
////                      NSData *return_services = [NSJSONSerialization dataWithJSONObject: [license objectForKey: @"services"] options: NSJSONWritingPrettyPrinted error: nil];
////                      NSMutableDictionary *services = [NSJSONSerialization JSONObjectWithData: return_services options:kNilOptions error: nil];
////                      for (NSDictionary *services in services)
////                      {
////                          
////                      }
////                      NSString *serviceName = [NSString stringWithFormat: @"%@", [license objectForKey: @""]];
//                  }
//                  else if ([return_code isEqualToString: @"400.5.2"])
//                  {
//                      dispatch_async(dispatch_get_main_queue(), ^() {
...
- (IBAction)detailEditBtn:(id)sender
{
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(2 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        [self.editViewSaveBtn setEnabled: NO];
...
- (void)renewActivateBtn:(UIButton *)sender
{
    ...
    [self activeLicense: detailDeviceId andServiceId: [renewServiceId objectAtIndex: sender.tag]];
}
...
{
    if (![self.editViewDevieNameTxt.text isEqualToString: detailDeviceName])
    {
        [self.editViewSaveBtn setEnabled: YES];
    }
    else
    {
        [self.editViewSaveBtn setEnabled: NO];
                ...
                if (_scanStatus)
                {
                    if ([metadataObject.stringValue length] == 33)
                    {
                        ...
                        if ([snKey isEqualToString: @"SN:"] && [macKey isEqualToString: @"MAC:"] && ([dataConvert length] == 33))
                        {
                            ...
                            [self checkNewDevice: macConvert andSn: [sn objectAtIndex: 1]];
                            [self.scanErrorMessage setHidden: YES];
                        }
                    }
                    else
...
- (NSIndexPath *)tableView:(UITableView *)tableView willSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (deviceListStatus == YES) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^() {
            [self getDeviceDetailInfo: [NSString stringWithFormat: @"%ld", (long)deviceCell.name.tag]];
        });
    }
    return indexPath;
}
