    ...
    [[session dataTaskWithRequest: request_user_info
                completionHandler:^(NSData *data, NSURLResponse *response, NSError *error)
      {
          if (data != nil)
          {
              ...
              else if ([code isEqualToString: @"400.5.16"]) // boundle licnese
              {
                  NSString *encryptionCode = [json objectForKey: @"data"];
                  NSString *iv = [encryptionCode substringWithRange: NSMakeRange(0, 16)];
                  NSString *encrypted_data = [encryptionCode substringFromIndex: 16];
        ...
        for (int j=0; j<[serviceCodeList count]; j++) {
            switch (deviceType)
            {
                case 1:
                    ...
                    amount = [[UILabel alloc]initWithFrame: CGRectMake(145, j*40, 120, 20)];
                    [contentView setContentSize: CGSizeMake(265, (j*40)+20)];
                    break;
                case 2:
                    code = [[UILabel alloc]initWithFrame: CGRectMake(35, j*40, 100, 20)];
                    amount = [[UILabel alloc]initWithFrame: CGRectMake(180, j*40, 140, 20)];
...
- (void)searchDeviceInfo
{
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        keyWord = [self.searchDevicesText.text stringByTrimmingCharactersInSet: [NSCharacterSet whitespaceAndNewlineCharacterSet]];;
        if ([keyWord length] > 0)
        {
